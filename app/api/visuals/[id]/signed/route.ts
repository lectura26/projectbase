...
export async function GET(
  ...
  try {
    ...
    if (!rateLimitAllow(`api-visual-signed:${ip}:${user.id}`, SIGNED_GET_MAX, SIGNED_GET_WINDOW_MS)) {
    ...
    const signed = await createSignedStorageUrl(visual.storagePath, SIGNED_URL_TTL_SEC);
    if ("error" in signed) {
      console.error("GET /api/visuals/[id]/signed createSignedStorageUrl:", signed.error);
      return jsonError(500, signed.error);
    }

    return NextResponse.redirect(signed.signedUrl);
  } catch (err) {
    logServerError("GET /api/visuals/[id]/signed", err);
